...
VideoPC.prototype.render = function (_parent) {
  ...
  var n_choices = 5;
  var names = ['Left is clearly better', 'Left is slightly better', 'They are equal', 'Right is slightly better', 'Right is clearly better'];
  var radio_buttons = [];

  for (let i = 0; i < n_choices; ++i) {
    var choice = $(`<label class="radio-inline"><input type='radio' id='video${i}' name='selection'>${names[i]}</label>`);
    choice.click(function(){
          global.ratings[0].value = 2 - i;
          global.ratings[1].value = -2 + i;
          if(global.watched){
  ...
  for (var i = 0; i < n_choices; ++i) {
    var cb_element = $(`<div id='cb${i}' class=inline-block></div>`);
...
VideoPC.prototype.postCheck = function() {
  ...
  if (atKey){
    var atTarget = 0
    if (atKey.includes('Left_is_clearly_better')){
      atTarget = 2;
    }else if (atKey.includes('Left_is_slightly_better')){
      atTarget = 1;
    }else if (atKey.includes('They_are_equal')){
      atTarget = 0;
    }else if (atKey.includes('Right_is_slightly_better')){
      atTarget = -1;
    }else if (atKey.includes('Right_is_clearly_better')){
      atTarget = -2;
    }
